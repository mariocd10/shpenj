    ...
    window.formatGoogleCalendar = function () {
        ...
        var transformationList = function transformationList(result, tagName, format) {
            ...
            for (i = 0; i < format.length; i++) {
                ...
                if (format[i] === '*summary*') {
                    output = output.concat('<h5 class="card-header summary">' + summary + '</h5>');
                } else if (format[i] === '*date*') {
                    output = output.concat('<h5 class="card-title date">' + dateFormatted + '</h5>');
        ...
        return {
            init: function init(settingsOverride) {
                var settings = {
                    ...
                    upcomingHeading: '<h2 class="col-md-12 heading">Upcoming Events</h2>',
                    pastHeading: '<h2 class="col-md-12 heading">Past events</h2>',
